...
from python_motion_planning.common.geometry import BSpline
...
class PSO(Planner):
    ...
    def __init__(self, params: dict) -> None:
        ...
        if self.agent_num != 1:
            raise RuntimeError("The number of agents must be equal to 1.")

        # plannig start and goal
        agent = list(self.agent_info.values())[0]
        start = agent["start"] if len(agent["start"]) == 2 else (agent["start"][0], agent["start"][1])
        goal = agent["goal"] if len(agent["goal"]) == 2 else (agent["goal"][0], agent["goal"][1])
