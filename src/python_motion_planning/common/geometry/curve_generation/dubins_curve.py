...
class Dubins(Curve):
	...
	def interpolate(self, mode: str, length: float, init_pose: Point3d):
		"""
		...
		Parameters:
			...
			init_pose (Point3d): Initial pose (x, y, yaw)

		Returns:
			new_pose (Point3d): New pose (new_x, new_y, new_yaw) after moving
		"""
		x, y, yaw = init_pose
		...
		return Point3d(new_x, new_y, new_yaw)
	...
	def generation(self, start_pose: Point3d, goal_pose: Point3d):
		...
		for mode_, seg_length in zip(best_mode, segments):
			...
			while abs(length) <= abs(seg_length):
				i += 1
				x_list[i], y_list[i], yaw_list[i] = self.interpolate(mode_, length, Point3d(x, y, yaw))
				length += d_length
			i += 1
			x_list[i], y_list[i], yaw_list[i] = self.interpolate(mode_, seg_length, Point3d(x, y, yaw))
	...
	def run(self, points: List[Point3d]) -> dict:
		"""
		...
		for i in range(len(points) - 1):
			_, _, x_list, y_list, yaw_list = self.generation(points[i], points[i + 1])
		...
		return [path_x, path_y, path_yaw], [
			...
			{"type": "marker", "name": "normal", "props": {"marker": "x", "color": "#ff0000"},
