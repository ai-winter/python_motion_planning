...
class QuinticPolynomial:
    ...
    def solve(self, s_pva: tuple, e_pva: tuple, t: float) -> None:
        x0, v0, a0 = s_pva
        xt, vt, at = e_pva
    ...
    def x(self, t):
        return self.params[0] + self.params[1] * t + self.params[2] * t ** 2 + self.params[3] * t ** 3 \
               + self.params[4] * t ** 4 + self.params[5] * t ** 5

    def dx(self, t):
        return self.params[1] + 2 * self.params[2] * t + 3 * self.params[3] * t ** 2 \
               + 4 * self.params[4] * t ** 3 + 5 * self.params[5] * t ** 4

    def ddx(self, t):
        return 2 * self.params[2] + 6 * self.params[3] * t + 12 * self.params[4] * t ** 2 \
               + 20 * self.params[5] * t ** 3

    def dddx(self, t):
        return 6 * self.params[3] + 24 * self.params[4] * t + 60 * self.params[5] * t ** 2

    def ddddx(self, t):
        return 24 * self.params[4] + 120 * self.params[5] * t
...
class Polynomial(Curve):
    ...
    def generation(self, start_pose: tuple, goal_pose: tuple):
        ...
        Parameters:
            start_pose (tuple): Initial pose (x, y, yaw, v, a)
            goal_pose (tuple): Target pose (x, y, yaw, v, a)
        ...
        for T in np.arange(self.t_min, self.t_max, self.step):
            x_psolver = QuinticPolynomial([0 for _ in range(6)])
            x_psolver.solve((sx, sv_x, sa_x), (gx, gv_x, ga_x), T)
            y_psolver = QuinticPolynomial([0 for _ in range(6)])
            y_psolver.solve((sy, sv_y, sa_y), (gy, gv_y, ga_y), T)
    ...
    def run(self, points: List[Point3d]):
        ...
        for i in range(len(points) - 1):
            path = self.generation(
				(points[i].x(), points[i].y(), points[i].theta(), v[i], a[i]),
				(points[i + 1].x(), points[i + 1].y(), points[i + 1].theta(), v[i + 1], a[i + 1])
            )
