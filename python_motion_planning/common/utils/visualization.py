...
from python_motion_planning.common.structure import Env, Grid
...
class Visualizer:
    def __init__(self, fig_name: str):
        ...
        # colors
        self.cmap_dict = {
            "free": ColorInfo(0, "#ffffff"),
            "obstacle": ColorInfo(1, "#000000"),
            "start": ColorInfo(2, "#ff0000"),
            "goal": ColorInfo(3, "#1155cc"),
            "custom": ColorInfo(4, "#dddddd"),
        }
        self.cmap = mcolors.ListedColormap([info.color for info in self.cmap_dict.values()])
        self.norm = mcolors.BoundaryNorm([i for i in range(self.cmap.N + 1)], self.cmap.N)
        self.grid_map = None

    def plotGridMap(self, env: Grid) -> None:
        '''
        Plot grid map with static obstacles.

        Parameters
        ----------
        env: grid map handler
        '''
